...
parser.add_argument("--save_image_interval", type=int, default=250, help='iteration interval to save image')
...
if args.mixed_precision == 'no':
    ...
    acc_common_args = '--mixed_precision="bf16"'# --use_fsdp --fsdp_offload_params="False" --fsdp_sharding_strategy=2'
...
if args.world_size != '1':
    ...
    run_cmd = (f'accelerate launch {acc_common_args} '
               ...
               f'train_scripts/train_flowie.py {main_args}'
               )
else:
    run_cmd = (f'accelerate launch {acc_common_args} '
               f'--num_machines={args.world_size} '
               f'--num_processes={num_gpus_per_node} '
               f'--main_process_port=29509 '
               'train_scripts/train_flowie.py '
               f'{main_args}'
               )
