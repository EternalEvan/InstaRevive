...
def extract_caption_t5_job(item):
    ...
    with torch.no_grad():
        ...
        save_path = os.path.join(t5_save_dir, item[0][:-4])
        if os.path.exists(save_path + ".npz"):
            count += 1
            return

        caption = item[1].strip()
...
def get_args():
    ...
    parser.add_argument('--t5_save_root', default='/data1/zyx/testsets/captions', type=str)
