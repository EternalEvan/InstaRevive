...
from dataset.codeformer import CodeformerDataset,CodeformerDatasetLQ,CodeformerDatasetLQ_prompts
...
train_dataset = CodeformerDatasetLQ_prompts(
        lq_list='/data1/zyx/testsets/realsrset.list', #/data3/zyx/FFHQ512/celeba-test.list',
        out_size=512,
        crop_type="center",
        use_hflip=True,
        blur_kernel_size=41,
        ...
        file_dir='/data1/zyx/testsets/testdata/RealSet65/'
        )
...
state_dict = torch.load('/data1/zyx/pixart/general-prompt-real_dm_regression_1distep_constant1e-06sgmul1.0warmup0_cfg3.0_999ts_acc2_maxgrad10.0_mixedprecisionfp16_bs4_one_step_maxt400/checkpoint-30000/pytorch_model.bin')
...
y_null_all = torch.load("/home/zyx/PixArt-sigma/output/tmp/real-world image, realistic, high quality, photograph, film, professional, 4k, highly detailed_300token.pth", map_location="cpu")
...
output_dir = '/data1/zyx/pixart/output/RealSet65-dmd'
cond_dir = '/data1/zyx/pixart/output/RealSet65-cond'
...
for step, batch in enumerate(train_dataloader):
    ...
    with torch.no_grad():
        ...
        # y = batch['txt_fea'].to(device).to(torch.float32)
        # y_mask = batch['attention_mask'].to(device).to(torch.float32)
        ...
        condition = cond['c_concat'][0]/2+0.5
        save_batch(images,imgname,output_dir)
        save_batch(condition,imgname,cond_dir)
        # print(noisy_latents)
        # print(timesteps)
